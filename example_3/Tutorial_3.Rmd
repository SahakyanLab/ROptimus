...
The process of determining the energy of a configuration corresponding to a given set of angles $\psi, \phi$ is the most computationally intensive part of this optimisation formulation. Having defined the necessary inputs for Optimus, it should be apparent this calculation will entirely be handled by MOPAC. This ability to serve as an optimisation kernel by off-loading a significant amount of computation to an external program is one of the many strengths of Optimus.
...
We will assume that the above listed conformations represent all conformational local minima in the parameter space of $\psi$ and $\phi$. Under this assumption, Conformation 1 should be considred the ground state conformation of Vitamin C. The accuracy of the results produced by Optimus can thus be judged by comparing them to the data listed in this table. It is important to recognize that the "resolution" of $\psi$ and $\phi$ when being optimized by Optimus is 2 degrees due to the manner in which r() was defined. As such, results produced by Optimus that are within plus or minus 2 degrees of a reference conformation should be tolerated.
...
Processors 1, 3 and 4 all arrived at a conformation defined by $\{\phi = 50, \psi = -172\}$, with an energy of $-232.874$ kcal/mol. The below Newman Projections and 3D structure depict this solution:

# INSERT 50, 188 NEWMAN PROJECTIONS HERE !!!!!!!!!!!!!!!!

# INSERT 50, 188 3D Structure !!!!!!!!!!!!!!!!!!
...
The below graphs illustrate the system psuedo-temperature and observed acceptance ratio for the last 20 000 optimisation iterations executed by Processor 3.
...
c(90, 82, 74, 66, 58, 50, 42, 34, 26, 18, 10, 2)
...
Of the 12 replicas, Processor 4 recovered the conformation with the lowest energy ($-233.1979$), defined by $\{\phi = 90, \psi = 76\}$. The below Newman Projections and 3D structure depict this solution:

# INSERT 90, 76 NEWMAN PROJECTIONS HERE !!!!!!!!!!!!!!!!

# INSERT 90, 76 3D Structure !!!!!!!!!!!!!!!!!!
...
When the optimisation process is first initialized, it is very unlikely that the input initial temperature is conducive to the target acceptance ratio. As such, the Temperature Control Unit alters the system pseudo-temperature considerably and rapidly to align the observed acceptance ratio with the target acceptance ratio, as can be seen in the above two graphs. Moreover, as stated in the previous Tutorial, an exchange between two replicas often has a similar effect of introducing a parameter configuration that is not conducive to the current system pseudo-temperature, which catalyzes significant temperature adjustments executed by the TCU. Accoridngly, sharp increases or decreased in the system pseudo-temperature and significant changes in the value around which the observed acceptance ratio oscillates in the graph above likely indicate steps at which an exchange involving replica 4 occurred.
